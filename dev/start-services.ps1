...
$Containers = @(
    ...
    @{ Name='unifiedops-influx-health-report-cdvl'; Port=$env:BROCADE_CDVL_REPORT_PORT; Bucket=$env:BROCADE_CDVL_REPORT_BUCKET; Token=$env:BROCADE_CDVL_REPORT_TOKEN },
    @{ Name='unifiedops-influx-health-report-bcp';  Port=$env:BROCADE_BCP_REPORT_PORT;  Bucket=$env:BROCADE_BCP_REPORT_BUCKET;  Token=$env:BROCADE_BCP_REPORT_TOKEN  }
)
...
if (-not $SkipPodman) {
    ...
    Write-Host '  starting 16 containers ...'
    foreach ($c in $Containers) {
        Write-Host ('    {0,-44}  :{1}' -f $c.Name, $c.Port)
        Ensure-Container -Cfg $c
    }

    Write-Host '  waiting for /health on all 16 endpoints ...'
    $allHealthy = $true
    foreach ($c in $Containers) {
    ...
    if ($allHealthy) {
        Write-Host '  all 16 InfluxDB instances healthy' -ForegroundColor Green
        Write-Host '  spinning off health-check-report buckets...' -ForegroundColor Cyan
        & podman exec unifiedops-influx-brocade-cdvl influx bucket create -n $env:BROCADE_CDVL_REPORT_BUCKET -o $env:INFLUX_ORG -t $env:BROCADE_CDVL_TOKEN 2>&1 | Out-Null
        & podman exec unifiedops-influx-brocade-bcp influx bucket create -n $env:BROCADE_BCP_REPORT_BUCKET -o $env:INFLUX_ORG -t $env:BROCADE_BCP_TOKEN 2>&1 | Out-Null
    } else {
...
$Listeners = @(
    ...
    @{ Name='brocade-cdvl-sify'; Script='syslog_trap_listener_cdvl_n_sify.py'; Port='5214'; Url="http://127.0.0.1:$($env:BROCADE_CDVL_PORT)"; Bucket=$env:BROCADE_CDVL_BUCKET; Token=$env:BROCADE_CDVL_TOKEN; HbUrl="http://127.0.0.1:$($env:HEARTBEAT_CDVL_PORT)"; HbBucket=$env:HEARTBEAT_CDVL_BUCKET; HbToken=$env:HEARTBEAT_CDVL_TOKEN; ReportUrl="http://127.0.0.1:$($env:BROCADE_CDVL_REPORT_PORT)"; ReportBucket=$env:BROCADE_CDVL_REPORT_BUCKET; ReportToken=$env:BROCADE_CDVL_REPORT_TOKEN },
    @{ Name='brocade-bcp-uat'; Script='syslog_trap_listener_bcp_n_uat.py'; Port='5215'; Url="http://127.0.0.1:$($env:BROCADE_BCP_PORT)"; Bucket=$env:BROCADE_BCP_BUCKET; Token=$env:BROCADE_BCP_TOKEN; HbUrl="http://127.0.0.1:$($env:HEARTBEAT_BCP_PORT)"; HbBucket=$env:HEARTBEAT_BCP_BUCKET; HbToken=$env:HEARTBEAT_BCP_TOKEN; ReportUrl="http://127.0.0.1:$($env:BROCADE_BCP_REPORT_PORT)"; ReportBucket=$env:BROCADE_BCP_REPORT_BUCKET; ReportToken=$env:BROCADE_BCP_REPORT_TOKEN },
...
foreach ($l in $Listeners) {
    ...
    $cmd = "`$env:HITRACK_INFLUX_URL='$($l.Url)'; `$env:HITRACK_INFLUX_TOKEN='$($l.Token)'; `$env:HITRACK_INFLUX_ORG='HDFC'; `$env:HITRACK_INFLUX_BUCKET='$($l.Bucket)'; `$env:HITRACK_LISTEN_HOST='127.0.0.1'; `$env:HITRACK_LISTEN_PORT='$($l.Port)'; `$env:HITRACK_TEST_MODE='1'; `$env:HITRACK_HEARTBEAT_URL='$($l.HbUrl)'; `$env:HITRACK_HEARTBEAT_TOKEN='$($l.HbToken)'; `$env:HITRACK_HEARTBEAT_ORG='HDFC'; `$env:HITRACK_HEARTBEAT_BUCKET='$($l.HbBucket)'; `$env:HITRACK_HEARTBEAT_INTERVAL='10'; "
    if ($l.ReportUrl) {
        $cmd += "`$env:HITRACK_INFLUX_REPORT_URL='$($l.ReportUrl)'; `$env:HITRACK_INFLUX_REPORT_TOKEN='$($l.ReportToken)'; `$env:HITRACK_INFLUX_REPORT_BUCKET='$($l.ReportBucket)'; "
    }
    $cmd += "& '$Venv' '$Root\listener\$($l.Script)'"
...
$backendCmd = "`$env:HITRACK_UI_HOST='$env:BACKEND_HOST'; `$env:HITRACK_UI_PORT='$env:BACKEND_PORT'; `$env:HITRACK_BUCKET_HITACHI_CDVL_URL='http://127.0.0.1:$env:HITACHI_CDVL_PORT'; `$env:HITRACK_BUCKET_HITACHI_CDVL_TOKEN='$env:HITACHI_CDVL_TOKEN'; `$env:HITRACK_BUCKET_HITACHI_CDVL_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_HITACHI_CDVL_BUCKET='$env:HITACHI_CDVL_BUCKET'; `$env:HITRACK_BUCKET_HITACHI_BCP_URL='http://127.0.0.1:$env:HITACHI_BCP_PORT'; `$env:HITRACK_BUCKET_HITACHI_BCP_TOKEN='$env:HITACHI_BCP_TOKEN'; `$env:HITRACK_BUCKET_HITACHI_BCP_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_HITACHI_BCP_BUCKET='$env:HITACHI_BCP_BUCKET'; `$env:HITRACK_BUCKET_HITACHI_SIFY_URL='http://127.0.0.1:$env:HITACHI_SIFY_PORT'; `$env:HITRACK_BUCKET_HITACHI_SIFY_TOKEN='$env:HITACHI_SIFY_TOKEN'; `$env:HITRACK_BUCKET_HITACHI_SIFY_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_HITACHI_SIFY_BUCKET='$env:HITACHI_SIFY_BUCKET'; `$env:HITRACK_BUCKET_BROCADE_CDVL_URL='http://127.0.0.1:$env:BROCADE_CDVL_PORT'; `$env:HITRACK_BUCKET_BROCADE_CDVL_TOKEN='$env:BROCADE_CDVL_TOKEN'; `$env:HITRACK_BUCKET_BROCADE_CDVL_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_BROCADE_CDVL_BUCKET='$env:BROCADE_CDVL_BUCKET'; `$env:HITRACK_INFLUX_BROCADE_CDVL_REPORT_URL='http://127.0.0.1:$env:BROCADE_CDVL_REPORT_PORT'; `$env:HITRACK_INFLUX_BROCADE_CDVL_REPORT_TOKEN='$env:BROCADE_CDVL_REPORT_TOKEN'; `$env:HITRACK_INFLUX_BROCADE_BCP_REPORT_URL='http://127.0.0.1:$env:BROCADE_BCP_REPORT_PORT'; `$env:HITRACK_INFLUX_BROCADE_BCP_REPORT_TOKEN='$env:BROCADE_BCP_REPORT_TOKEN'; `$env:HITRACK_BUCKET_BROCADE_BCP_URL='http://127.0.0.1:$env:BROCADE_BCP_PORT'; `$env:HITRACK_BUCKET_BROCADE_BCP_TOKEN='$env:BROCADE_BCP_TOKEN'; `$env:HITRACK_BUCKET_BROCADE_BCP_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_BROCADE_BCP_BUCKET='$env:BROCADE_BCP_BUCKET'; `$env:HITRACK_BUCKET_NETAPP_CDVL_URL='http://127.0.0.1:$env:NETAPP_CDVL_PORT'; `$env:HITRACK_BUCKET_NETAPP_CDVL_TOKEN='$env:NETAPP_CDVL_TOKEN'; `$env:HITRACK_BUCKET_NETAPP_CDVL_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_NETAPP_CDVL_BUCKET='$env:NETAPP_CDVL_BUCKET'; `$env:HITRACK_BUCKET_NETAPP_BCP_URL='http://127.0.0.1:$env:NETAPP_BCP_PORT'; `$env:HITRACK_BUCKET_NETAPP_BCP_TOKEN='$env:NETAPP_BCP_TOKEN'; `$env:HITRACK_BUCKET_NETAPP_BCP_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_NETAPP_BCP_BUCKET='$env:NETAPP_BCP_BUCKET'; `$env:HITRACK_BUCKET_NETAPP_SIFY_URL='http://127.0.0.1:$env:NETAPP_SIFY_PORT'; `$env:HITRACK_BUCKET_NETAPP_SIFY_TOKEN='$env:NETAPP_SIFY_TOKEN'; `$env:HITRACK_BUCKET_NETAPP_SIFY_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_NETAPP_SIFY_BUCKET='$env:NETAPP_SIFY_BUCKET'; `$env:HITRACK_BUCKET_DELL_CDVL_URL='http://127.0.0.1:$env:DELL_CDVL_PORT'; `$env:HITRACK_BUCKET_DELL_CDVL_TOKEN='$env:DELL_CDVL_TOKEN'; `$env:HITRACK_BUCKET_DELL_CDVL_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_DELL_CDVL_BUCKET='$env:DELL_CDVL_BUCKET'; `$env:HITRACK_BUCKET_DELL_BCP_URL='http://127.0.0.1:$env:DELL_BCP_PORT'; `$env:HITRACK_BUCKET_DELL_BCP_TOKEN='$env:DELL_BCP_TOKEN'; `$env:HITRACK_BUCKET_DELL_BCP_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_DELL_BCP_BUCKET='$env:DELL_BCP_BUCKET'; `$env:HITRACK_BUCKET_DELL_SIFY_URL='http://127.0.0.1:$env:DELL_SIFY_PORT'; `$env:HITRACK_BUCKET_DELL_SIFY_TOKEN='$env:DELL_SIFY_TOKEN'; `$env:HITRACK_BUCKET_DELL_SIFY_ORG='$env:INFLUX_ORG'; `$env:HITRACK_BUCKET_DELL_SIFY_BUCKET='$env:DELL_SIFY_BUCKET'; `$env:HITRACK_HEARTBEAT_CDVL_URL='http://127.0.0.1:$env:HEARTBEAT_CDVL_PORT'; `$env:HITRACK_HEARTBEAT_CDVL_TOKEN='$env:HEARTBEAT_CDVL_TOKEN'; `$env:HITRACK_HEARTBEAT_CDVL_ORG='$env:INFLUX_ORG'; `$env:HITRACK_HEARTBEAT_CDVL_BUCKET='$env:HEARTBEAT_CDVL_BUCKET'; `$env:HITRACK_HEARTBEAT_BCP_URL='http://127.0.0.1:$env:HEARTBEAT_BCP_PORT'; `$env:HITRACK_HEARTBEAT_BCP_TOKEN='$env:HEARTBEAT_BCP_TOKEN'; `$env:HITRACK_HEARTBEAT_BCP_ORG='$env:INFLUX_ORG'; `$env:HITRACK_HEARTBEAT_BCP_BUCKET='$env:HEARTBEAT_BCP_BUCKET'; `$env:HITRACK_HEARTBEAT_SIFY_URL='http://127.0.0.1:$env:HEARTBEAT_SIFY_PORT'; `$env:HITRACK_HEARTBEAT_SIFY_TOKEN='$env:HEARTBEAT_SIFY_TOKEN'; `$env:HITRACK_HEARTBEAT_SIFY_ORG='$env:INFLUX_ORG'; `$env:HITRACK_HEARTBEAT_SIFY_BUCKET='$env:HEARTBEAT_SIFY_BUCKET'; `$env:HITRACK_VERIFY_TLS='$env:HITRACK_VERIFY_TLS'; `$env:HITRACK_PIPELINE_POLL_SECS='$env:PIPELINE_POLL_SECS'; `$env:HITRACK_LISTENER_POLL_SECS='$env:LISTENER_POLL_SECS'; `$env:HITRACK_LISTENER_DOWN_THRESHOLD_S='$env:LISTENER_DOWN_THRESHOLD_S'; & '$Venv' '$Root\server\server.py' *> '$Root\server.log'"
...
Write-Host ("  InfluxDB:    {0,2}/16 containers up" -f ($running | Measure-Object).Count)
