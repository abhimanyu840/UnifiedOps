...
"""
=============================================================================
  Syslog Trap Listener -> InfluxDB v2  (Hi-Track / HDFC -- CDVL pipeline)
=============================================================================

Self-contained listener for the CDVL location. The engine logic and the
CDVL IP maps live in this single file - no shared modules, no other
location referenced.

Configuration overrides (typical: /etc/hi-track/listener.cdvl.env):

    HITRACK_INFLUX_URL      default http://127.0.0.1:8086
    HITRACK_INFLUX_TOKEN    *** required for writes ***
    HITRACK_INFLUX_ORG      default HDFC
    HITRACK_INFLUX_BUCKET   default SYSLOG_HIT_Bucket
    HITRACK_LISTEN_HOST     default 0.0.0.0
    HITRACK_LISTEN_PORT     default 514          (TCP and UDP share same port)
...
INFLUX_BUCKET = os.environ.get("HITRACK_INFLUX_BUCKET", "SYSLOG_HIT_Bucket")
...
# WORKER_THREADS:  parse+write workers behind the UDP/TCP recv loops so a
#                  slow Influx write never blocks new incoming packets.
# WRITE_BATCH:     enable batched async writes to Influx; reduces per-packet
#                  HTTP overhead under bursty loads. Set HITRACK_WRITE_BATCH=0
#                  to fall back to the legacy SYNCHRONOUS mode.
# WRITE_BATCH_SIZE / WRITE_FLUSH_MS / WRITE_JITTER_MS / WRITE_RETRY_INTERVAL_MS:
#                  influxdb_client batch tuning. Defaults trade ~1s of write
#                  latency for ~10x throughput vs SYNCHRONOUS.
...
TEST_DEFAULT_IP = os.environ.get("HITRACK_TEST_DEFAULT_IP", "10.227.60.189")
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("syslog_trap_listener_cdvl.log"),
    ],
)
log = logging.getLogger("syslog_trap_listener_cdvl")

raw_log = logging.getLogger("raw_syslog_cdvl")
raw_log.setLevel(logging.INFO)
raw_fh = logging.FileHandler("syslog_trap_listener_cdvl_raw_syslog_data.log")
raw_fh.setFormatter(logging.Formatter("%(asctime)s %(message)s"))
raw_log.addHandler(raw_fh)
raw_log.propagate = False

# ---------------------------------------------------------------------------
# Heartbeat — inline per-listener (each listener is its own service)
# Writes a `syslog_listener_heartbeat` point into the per-site heartbeat
# InfluxDB every HB_INTERVAL seconds so the dashboard can detect a dead
# listener even when its main Influx is still reachable.
# ---------------------------------------------------------------------------
...
IP_FILTER: dict[str, str] = {
    "10.227.60.189":  "modular_storage",
    "10.227.60.190":  "modular_storage",
    "10.227.61.127":  "modular_storage",
    "10.227.61.128":  "modular_storage",
    "10.227.61.130":  "modular_storage",
    "10.227.61.131":  "modular_storage",
    "10.227.61.56":   "modular_storage",
    "10.227.61.53":   "modular_storage",
    "10.227.61.54":   "modular_storage",
    "10.227.61.57":   "modular_storage",
    "10.227.61.50":   "modular_storage",
    "10.227.61.51":   "modular_storage",
    "10.227.62.222":  "modular_storage",
    "10.227.62.221":  "modular_storage",
    "10.227.68.117":  "modular_storage",
    "10.227.68.118":  "modular_storage",
    "10.229.230.232": "modular_storage",
    "10.229.230.233": "modular_storage",
    "10.227.71.50":   "modular_storage",
    "10.227.71.51":   "modular_storage",
    "10.227.68.120":  "modular_storage",
    "10.227.68.121":  "modular_storage",
    "10.227.68.123":  "modular_storage",
    "10.227.68.124":  "modular_storage",
    "10.227.68.126":  "modular_storage",
    "10.227.68.127":  "modular_storage",
    "10.227.68.129":  "modular_storage",
    "10.227.68.130":  "modular_storage",
    "10.227.68.132":  "modular_storage",
    "10.227.68.133":  "modular_storage",
    "10.1.40.8":      "modular_storage",
    "10.1.40.9":      "modular_storage",
    "10.1.40.11":     "modular_storage",
    "10.1.40.12":     "modular_storage",

    "10.5.7.228":     "enterprise_storage",
    "10.5.7.229":     "enterprise_storage",
    "10.5.7.230":     "enterprise_storage",
    "10.5.7.231":     "enterprise_storage",
    "10.227.61.125":  "enterprise_storage",
    "10.227.63.5":    "enterprise_storage",
    "10.227.63.80":   "enterprise_storage",
    "10.227.63.141":  "enterprise_storage",
    "10.226.116.16":  "enterprise_storage",
    "10.226.116.17":  "enterprise_storage",
    "10.226.116.18":  "enterprise_storage",
    "10.226.117.2":   "enterprise_storage",
}

# ---------------------------------------------------------------------------
# IP -> friendly storage name (CDVL only). Each VSP storage box has multiple
# management IPs; they all map to the same -CDVL-suffixed hostname so every
