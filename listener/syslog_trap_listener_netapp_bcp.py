...
Standalone listener for the BCP NetApp pipeline. Receives NetApp ONTAP
EMS syslog forwards on UDP/TCP and writes parsed events into a
dedicated Influx bucket on the local BCP VM.
...
logging.basicConfig(
    ...
    format=f"%(asctime)s [%(levelname)s] netapp-bcp: %(message)s",
)
...
def main():
    LOG.info("=" * 60)
    LOG.info(" NetApp Syslog Listener (BCP) - starting up")
