...
Standalone listener for the SIFY NetApp pipeline. Receives NetApp ONTAP
EMS syslog forwards on UDP/TCP and writes parsed events into a
dedicated Influx bucket on the local SIFY VM.

Placeholder parser — binding + Influx writer are production-grade, but
the trap-body parsing is intentionally minimal until the real NetApp
EMS schema is reverse-engineered for this site.

Configuration overrides (typical: /etc/hi-track/listener.netapp.sify.env):

    HITRACK_INFLUX_URL      default http://127.0.0.1:8288
...
INFLUX_URL    = os.environ.get("HITRACK_INFLUX_URL",    "http://127.0.0.1:8288")
...
logging.basicConfig(
    ...
    format=f"%(asctime)s [%(levelname)s] netapp-sify: %(message)s",
)
LOG = logging.getLogger("hitrack.listener.netapp.sify")
...
def main():
    LOG.info("=" * 60)
    LOG.info(" NetApp Syslog Listener (SIFY) - starting up")
